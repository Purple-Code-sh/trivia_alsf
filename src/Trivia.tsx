...
export default function Trivia() {
  ...
  return (
    <section className=' w-screen relative overflow-hidden py-12 md:py-16 lg:py-20 border min-h-[100vh] flex flex-col items-center'>
      <div className="container bg-zinc-200 flex flex-col items-center justify-center gap-8 md:gap-16 lg:gap-24">
        ...
        <div className='flex flex-wrap gap-8 md:gap-16 lg:gap-24 w-full items-stretch  justify-center'>
          ...
          <button className={`bg-white p-6 rounded-lg shadow-xl hover:scale-105 transition-transform duration-300  w-full max-w-[460px] flex flex-col items-center justify-start gap-6 md:gap-8 lg:gap-12 ${optionSelected === questions[currentQuestion].correctAnswer ? ' outline-green-500' : ' outline-accent-red'} ${hasSelected === 'a' ? ' outline-4 ' : ' outline-0'} ${optionSelected === 'a' && hasSelected === '' ? ' outline-4 outline-primary' : ' outline-0'}`} onClick={() => setOptionSelected('a')}>
            <h3 className='text-primary text-6xl pt-6 lg:text-7xl font-black animate-[bounce_5s_infinite]'>A</h3>
          ...
          <button className={`bg-white p-6 rounded-lg shadow-xl hover:scale-105 transition-transform duration-300  w-full max-w-[460px] flex flex-col items-center justify-start gap-6 md:gap-8 lg:gap-12 ${optionSelected === questions[currentQuestion].correctAnswer ? ' outline-green-500' : ' outline-accent-red'} ${hasSelected === 'b' ? ' outline-4' : ' outline-0'} ${optionSelected === 'b' && hasSelected === '' ? ' outline-4 outline-primary' : ' outline-0'}`} onClick={() => setOptionSelected('b')}>
            <h3 className='text-primary text-6xl pt-6 lg:text-7xl font-black animate-[bounce_5s_infinite]'>B</h3>
        ...
        {hasAnswered === false &&
          <button className={`bg-secondary py-4 px-8 hover:scale-110 active:underline transition-all duration-300 rounded-4xl underline-offset-4 cursor-pointer text-white font-semibold font-urbanist text-2xl ${optionSelected !== '' ? ' visible' : 'invisible'}`} onClick={() => {
            setHasSelected(optionSelected)
            setHasAnswered(true)
          }} >Responder</button>
        }

        {hasAnswered && <button className={`bg-accent-orange py-4 px-8 hover:scale-110 active:underline transition-all duration-300 rounded-4xl underline-offset-4 cursor-pointer text-txt-200 font-semibold font-urbanist text-2xl`} onClick={() => {
